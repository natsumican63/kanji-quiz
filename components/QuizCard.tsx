  ...
  return (
    <div className="w-full max-w-xl mx-auto bg-white rounded-[3rem] shadow-xl overflow-hidden border-8 border-yellow-300 transform transition-all duration-300">
      <div className="bg-yellow-100 p-6 md:p-10 text-center border-b-8 border-yellow-300">
        <p className="text-xl md:text-2xl font-bold text-orange-600 mb-4">この かんじは なにかな？</p>
            ...
            <div key={index} className="relative w-full">
              ...
              <button
                ...
                className={`
                  ...
                  w-full relative py-6 px-6 rounded-3xl text-2xl md:text-5xl font-black
                  shadow-lg transition-all duration-300 active:scale-95
                  flex items-center gap-4 min-h-[110px]
                `}
              >
                <span className="bg-white text-sky-600 w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center shadow-md text-xl md:text-3xl font-black shrink-0">
                  {index === 0 ? 'A' : 'B'}
                </span>
                <span className="text-left leading-tight overflow-hidden text-ellipsis">
                  {option}
                </span>
                {selectedAnswer && isCorrect && <span className="ml-auto text-3xl md:text-5xl animate-bounce shrink-0">⭕</span>}
              </button>
              ...
              {!selectedAnswer && (
                <button
                  ...
                  title="おとをきく"
                  className="absolute -top-2 -right-2 bg-pink-400 hover:bg-pink-500 text-white w-12 h-12 md:w-14 md:h-14 rounded-full shadow-lg active:scale-95 transition-all flex items-center justify-center border-2 border-white z-10"
                >
